...
func (peer *Peer) sendRandomPackets() {
	numPackets := crypto.GenerateRandomInt(8, 15)
	...
	for i := 0; i < numPackets; i++ {
		...
		time.Sleep(time.Duration(crypto.GenerateRandomInt(20, 100)) * time.Millisecond)
	}
}
...
func (peer *Peer) SendKeepalive() {
	if len(peer.queue.staged) == 0 && peer.isRunning.Load() {
		if peer.isCloudflareWarp {
			peer.device.log.Verbosef("%v - Trying WARP tricks! (keepalive)", peer)
...
func (peer *Peer) SendHandshakeInitiation(isRetry bool) error {
	...
	if peer.isCloudflareWarp {
		peer.device.log.Verbosef("%v - Trying WARP tricks! (handshake)", peer)
		peer.sendRandomPackets()
	}
