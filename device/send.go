...
func (peer *Peer) SendKeepalive() {
	if len(peer.queue.staged) == 0 && peer.isRunning.Load() {
		...
		// 	peer.sendRandomPackets(32, 64, 1, 2)
		// }
...
func (peer *Peer) SendHandshakeInitiation(isRetry bool) error {
	...
	if peer.tryUnblockWarp {
		if isRetry {
			peer.device.log.Verbosef("%v - Retrying a handshake to warp endpoint (with more oomph!): %v", peer, peer.endpoint.val.DstToString())
			peer.sendRandomPackets(128, 256, 2, 20)
		} else {
			peer.device.log.Verbosef("%v - Unblocking a handshake to warp endpoint: %v", peer, peer.endpoint.val.DstToString())
			peer.sendRandomPackets(64, 128, 2, 10)
		}
	}
