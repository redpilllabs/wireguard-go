...
func (device *Device) handlePeerLine(peer *ipcSetPeer, key, value string) error {
	...
	case "try_unblock_warp":
		isWarpUnblockerEnabled, err := strconv.ParseBool(value)
		if err != nil {
			return ipcErrorf(ipc.IpcErrorInvalid, "failed enable warp unblocker: %w", err)
		}
		if isWarpUnblockerEnabled {
			device.log.Verbosef("%v - UAPI: Enabling WARP unblocker", peer.Peer)
			peer.tryUnblockWarp = isWarpUnblockerEnabled
		}
